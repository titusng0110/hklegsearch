...
def main():
    ...
    local_model_path = "local_linq"

    # Check if model already exists locally, if not download and save it
    if not os.path.exists(local_model_path):
        print(f"Downloading model to {local_model_path}...")
        original_model = SentenceTransformer("Linq-AI-Research/Linq-Embed-Mistral", trust_remote_code=True, model_kwargs={ "torch_dtype": torch.float16 })
        original_model.save(local_model_path)
        print("Model saved locally")
        del original_model

    # Then in your main function, use the local path
    model = SentenceTransformer(local_model_path, trust_remote_code=False, model_kwargs={ "torch_dtype": torch.float16 })
    model.max_seq_length = 1024
    torch.cuda.empty_cache()
    pool = model.start_multi_process_pool(["cuda:0", "cuda:1", "cuda:2", "cuda:3"])
    embeddings = l2_normalize(model.encode_multi_process(df, pool, batch_size=24, show_progress_bar=True)).tolist()
    ...
    df_embeddings.write_parquet("linq_embeddings.parquet")
