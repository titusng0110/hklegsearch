...
def main():
    ...
    local_model_path = "local_inf_big"

    # Check if model already exists locally, if not download and save it
    if not os.path.exists(local_model_path):
        print(f"Downloading model to {local_model_path}...")
        original_model = SentenceTransformer("infly/inf-retriever-v1", trust_remote_code=True, model_kwargs={ "torch_dtype": torch.bfloat16 })
        original_model.save(local_model_path)
        print("Model saved locally")
        del original_model

    # Then in your main function, use the local path
    model = SentenceTransformer(local_model_path, trust_remote_code=False, model_kwargs={ "torch_dtype": torch.bfloat16 })
    ...
    df_embeddings.write_parquet("inf_big_embeddings.parquet")
